@if(isset($item))
    ...
    <div id="{{ $uniqid_01 }}_frame" style="display: none;">
        <iframe src="{{ url('/') }}/assets/admin/fileman/index.html?integration=custom&type=files&txtFieldId={{ $uniqid_01 }}"
    ...
    <div id="{{ $uniqid_02 }}_frame" style="display: none;">
        <iframe src="{{ url('/') }}/assets/admin/fileman/index.html?integration=custom&type=files&txtFieldId={{ $uniqid_02 }}"
                style="width:100%;height:100%" frameborder="0">
        </iframe>
    </div>
...
@else
    ...
    @include('work::admin._partials.upload',
        ['numOfItems' => 3, 'elementType' => 'video', 'labels' => ['Mp4', 'WMA', 'Ogg']]
    )
